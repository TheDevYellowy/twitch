...
module.exports = class API extends EventEmitter {
  ...
  async post(url, headers = {}, data) {
    ...
    if (res.status === 401 && !this.refresh_token)
      return "Token expired, if you want it to auto update please set refresh_token";
  ...
  async get(url, query = {}, headers = {}) {
    ...
    const res = await fetch(
      `https://api.twitch.tv/helix/${url}?${params.toString()}`,
      {
        method: "GET",
        headers: headers,
      }
  ...
  async resetToken(refresh = this.refresh_token, return_token = false) {
    const res = await fetch(
      `https://id.twitch.tv/oauth2/token?client_id=${this.client_id}&client_secret=${this.client_secret}&refresh_token=${refresh}&grant_type=refresh_token`,
      {
        method: "POST",
        headers: {
